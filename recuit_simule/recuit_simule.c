...
ruleSet_t *recuit_simule(ruleSet_t *rules, int T,
  ...
  while (T > 0) {
    newRules =
        changeRuleSet(rules); // Génération d'un nouvel ensemble de règles
    newRules->energy =
        energy(newRules); // Calcul de la performance de l'ensemble de règles
    if (newRules->energy < rules->energy)
      rules = newRules;
    rules->energy = energy(newRules);
    double p = exp((energy(rules) - energy(newRules)) / T);
    if (p >
        rand() / RAND_MAX) // Si le nouvel ensemble de règles n'améliore pas la
                           // performance, on le garde avec une probabilité p
      rules = newRules;
    rules->energy = energy(newRules);
    T--;
  }
