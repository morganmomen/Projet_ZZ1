        ...
        for (int j=0;j<(taille+2);j++)
        {
            ...
            else
            {
                if ((map[i][j]!=2)&&(map[i][j]!=3))
                {
                    ...
                    {
                        if
((((map[i+1][j]!=1)&&(map[i-1][j]!=1))&&((map[i][j-1]!=1)||(map[i][j+1]!=1)))&&(((map[i-1][j-1]!=1)||(map[i+1][j+1]!=1)))&&((map[i+1][j-1]!=1)||(map[i-1][j+1]!=1)))
                        {
                            if
(((map[i][j+1]!=3)&&((map[i+1][j]!=3)&&(map[i+1][j+1]!=3)))&&((map[i][j-1]!=2)&&(map[i-1][j]!=2)))
                            {
                                map[i][j]= 1;
                                compteur++;
                            }
...
void generateMaze_2chasseurs(int **maze, int taille, position * position_chasseur1,  position * position_chasseur2) {
  ...
  while (tailleChemin <= 2) {
    ...
    apparition_chasseur(maze, taille, position_chasseur1);
    apparition_chasseur(maze, taille, position_chasseur2);

    depart = (taille)*position_chasseur1->x + position_chasseur1->y;
    arriver = (taille)*position_lapin->x + position_lapin->y;
